...
-spec from_orddict(list({key(), value()})) -> tree().
%% @doc Create a tree from ordered dictionary.
%% This not necessarily results in a perfectly balanced tree, like the one obtained when running {@link balance/1}.
from_orddict(List) ->
    from_orddict(List, empty()).

-spec from_orddict(list({key(), value()}), Acc :: tree()) -> tree().
from_orddict([], Acc) ->
    Acc;
from_orddict([{Key, Value}|Rest], Acc) ->
    from_orddict(Rest, enter(Key, Value, Acc)).
...
-spec balance_list(list({key(), value()}), Size :: non_neg_integer()) -> tree_node().
balance_list(KVList, Size) ->
    {Node, []} = balance_list_1(KVList, Size),
    Node.

-spec balance_list_1(list({key(), value()}), Size :: non_neg_integer()) -> {tree_node(), list({key(), value()})}.
balance_list_1(List, Size) when Size > 1 ->
    Size2 = Size div 2,
    Size1 = Size - Size2,
    {LeftNode, List1=[{Key, _} | _]} = balance_list_1(List, Size1),
    {RightNode, List2} = balance_list_1(List1, Size2),
    InnerNode = {Key, inner_hash(node_hash(LeftNode), node_hash(RightNode)), LeftNode, RightNode},
    {InnerNode, List2};
balance_list_1([{Key, Value} | List], 1) ->
    {{Key, Value, leaf_hash(Key, Value)}, List};
balance_list_1(List, 0) ->
    {empty, List}.

-spec node_to_list(tree_node()) -> list({key(), value()}).
node_to_list(Node) ->
    foldr_1(
      fun (KV, Acc) ->
              [KV|Acc]
...
tree() ->
    %% The validity of data generated by this generator depends on the validity of the `from_orddict' function.
    %% This should not be a problem as long as the `from_orddict' function itself is tested.
    ?LET(KVO, kv_orddict(), from_orddict(KVO)).
non_empty_tree() ->
    ?SUCHTHAT(Tree, tree(), element(1, Tree) > 0).

%% Helper functions for PropEr.
...
node_shallow_height({_, _, Left, Right}) ->
    1 + min(node_height(Left), node_height(Right)).
...
prop_from_orddict_size() ->
    ?FORALL(KVO, kv_orddict(),
            length(KVO) =:= ?MODULE:size(from_orddict(KVO))).
...
prop_tree_after_explicit_balancing_is_perfectly_balanced() ->
    ?FORALL(Tree, tree(), height(balance(Tree)) - shallow_height(balance(Tree)) =< 1).
-endif.
